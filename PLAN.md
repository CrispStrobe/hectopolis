...
- [x] **T-401 Web release build** to `/mnt/storage/code/stadtbau/build/web`, deploy target
  ...
  *Note 2026-09-15:* Measured the first load: ~3.75 MB gzipped, of which CanvasKit's wasm is 2.9 MB and `main.dart.js` 0.9 MB. The Pages workflow now drops the engine `*.symbols` maps before upload — 8.2 MB of a 24 MB artifact, and nothing in `flutter.js` or the bundle ever requests one. A `--wasm` build (dart2wasm + skwasm, with the current dart2js + CanvasKit build kept as an automatic fallback for browsers without WasmGC) measures ~3.16 MB gzipped on a host that cannot set COOP/COEP, so ~16% less for modern browsers; not adopted, since it has not been loaded in a real browser here.
