...
- [x] **T-401 Web release build** to `/mnt/storage/code/stadtbau/build/web`, deploy target
  ...
  *Note 2026-09-15:* Measured a real first load in headless Chrome, reading the server access log. `flutter.js` loads the engine from `https://www.gstatic.com/flutter-canvaskit/<revision>/` unless the build sets `useLocalCanvasKit`, so our origin serves only ~883 KB gzipped (`main.dart.js`, bootstrap, manifests, icon font) and CanvasKit's ~2.9 MB gzipped comes from Google's CDN. The 22 MB `canvaskit/` directory Flutter writes next to it — every engine variant plus a `.symbols` map each — is never requested; the Pages workflow now drops it, 24 MB → 3 MB, guarded so it is kept if the build ever switches to `--no-web-resources-cdn`. The pruned build was loaded in headless Chrome and renders identically.
  *Two open decisions, both measured, neither taken here:* (1) `--no-web-resources-cdn` self-hosts the engine, removing the third-party request to gstatic.com on every first load — relevant to T-702's telemetry-free stance — at the cost of ~3.0 MB gzipped from our own origin instead of ~883 KB. (2) `--wasm` (dart2wasm + skwasm, with the dart2js + CanvasKit build kept as an automatic fallback) also loads and renders identically, but costs ~933 KB from our origin against ~883 KB, so it is a runtime-speed question, not a size one.
